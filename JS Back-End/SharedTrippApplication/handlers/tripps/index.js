        ...
        detailsTripp(req, res, next) {
            ...
            Tripp.findById(id).populate('buddies').lean().then((tripp) => {
                ...
                res.render('tripps/details-tripp.hbs', {
                    ...
                    userEmail: req.user ? req.user.email : '',
                    tripp,
                    isTheDriver: JSON.stringify(tripp.driver) === JSON.stringify(currentUser),
                    isAlreadyJoined: JSON.stringify(tripp.buddies).includes(JSON.stringify(currentUser)),
